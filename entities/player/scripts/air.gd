...
func physics_tick(delta: float) -> void:
	if player.velocity.y < 0:
		...
		if  scream_timer.is_stopped() and can_play and player.global_position.y < player.last_y - 5.0 and suv > 1.15 and suv < 1.25:
			scream.play(0.57)
			can_play = false
			scream_timer.start()
...
func suvat(u: float, a: float) -> float:
	...
	var query := PhysicsRayQueryParameters3D.create(player.global_position, player.global_position + Vector3.DOWN * 3000, 1, [player.get_rid()])
