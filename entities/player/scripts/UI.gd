...
func _ready() -> void:
	for rect: Cursor in [left_rect, right_rect, up_rect, down_rect]:
		...
		interact_ray.interact_start.connect(rect.tween_in)
		interact_ray.interact_end.connect(rect.tween_out)
		interact_ray.interact_exited.connect(rect.tween_default)
	interact_ray.interact_entered.connect(crosshair.tween_out)
	interact_ray.interact_start.connect(crosshair.tween_in)
	interact_ray.interact_end.connect(crosshair.tween_out)
