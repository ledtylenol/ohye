...
@export_range(-1.0, 1.0, 0.1)var k:= 1.0
@export_range(0.0, 1.0) var spread
@export_range(0.0, 1.0) var width
...
func _check_shader() -> bool:
	...
	if shader.is_valid() and shader_copy.is_valid() and pipeline.is_valid() and pipeline_copy.is_valid():
		return true
	# Compile our shader.
	var shader_file := load("res://compositor_effects/woah/dent.glsl")
	var shader_spirv: RDShaderSPIRV = shader_file.get_spirv()
