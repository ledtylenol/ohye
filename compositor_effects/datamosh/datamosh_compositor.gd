...
func _initialize_compute():
	...
	# Create our shader
	var shader_file = load("res://compositor_effects/datamosh/datamosh.glsl")
	...
	shader_file = load("res://compositor_effects/datamosh/loop_frame.glsl")
...
func get_image_buffer_uniform(image : RID, binding : int = 0) -> RDUniform:
	var uniform : RDUniform = RDUniform.new()
	...
	return uniform
func get_image_sampler_uniform( binding : int = 0) -> RDUniform:
	var uniform : RDUniform = RDUniform.new()
	...
	if rd and p_effect_callback_type == CompositorEffect.EFFECT_CALLBACK_TYPE_POST_TRANSPARENT:
		...
		if render_scene_buffers:
			...
			for view in range(view_count):
				...
				var depth_buffer = render_scene_buffers.get_depth_texture()
				...
				if not Global.refresh_frame:
					...
					uniform = get_image_buffer_uniform(depth_buffer)
					var o_u = get_image_sampler_uniform(1)
					var depth_set = UniformSetCacheRD.get_cache(datamosh_shader, 3, [uniform, o_u])
					...
					rd.compute_list_bind_uniform_set(compute_list, depth_set, 3)
